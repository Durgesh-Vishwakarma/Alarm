...
export async function isAlarmRunning() {
  return Boolean(await getActiveNativeAlarmId());
}

export async function startAlarmActivity(payload) {
  if (!hasNativeScheduler()) {
    return false;
  }

  if (typeof nativeScheduler.startAlarmActivity === 'function') {
    return nativeScheduler.startAlarmActivity(payload);
  }
...
export async function acquireWakeLock() {
  if (!hasNativeScheduler() || typeof nativeScheduler.acquireWakeLock !== 'function') {
    return false;
  }

  return nativeScheduler.acquireWakeLock();
}

export async function releaseWakeLock() {
  if (!hasNativeScheduler() || typeof nativeScheduler.releaseWakeLock !== 'function') {
    return false;
  }

  return nativeScheduler.releaseWakeLock();
}
